...
Task 23.1 adds the default-deny extension permission vocabulary and policy. Core defines permission categories for metadata, edit suggestions, export, limited filesystem access, AI results, and MCP modes. It does not define raw SQL or original-file mutation permissions.
...
Task 17.3 adds `get_photo_histogram` for the current committed supported-raster Develop state. Core records histogram JSON only as disposable render cache and returns explicit ready, missing, blocked, or unsupported states.
