...
Name: image
Version: 0.25.6
Purpose: JPEG/PNG/TIFF raster decode and export for the local alpha export path, ICC embedding for JPEG, thumbnail, Loupe preview, Develop preview cache generation, import-time supported-raster dimension inspection, and raster fixture inspection in integration tests.
License: MIT OR Apache-2.0
Repository/Homepage: https://github.com/image-rs/image
Used by: crates/silica-export at runtime; crates/silica-core and apps/desktop/src-tauri as dev-dependencies for raster test fixture generation and inspection.
Why needed: Task 5.4 must produce a real JPEG file, inspect the exported JPEG, and verify original files remain unchanged without implementing RAW decoding or the Metal viewer. Tasks 5.6.4, 5.6.5, and 5.6.6 reuse the same runtime image path to create disposable grid thumbnails, Loupe previews, and adjusted Develop previews for supported raster originals. Task 11.7.3 reuses `image::image_dimensions` to persist supported-raster width and height during import without adding an EXIF parser. Task 13.6 uses `ImageEncoder::set_icc_profile` for export ICC proof. Task 20.2 enables the existing `png` and `tiff` features so the same adjusted raster export path can write PNG and TIFF outputs. The 2026-06-27 source-contract update uses those existing features for PNG/TIFF source inputs as well as outputs.
...
Risk notes: Pinned exactly to 0.25.6 because it declares Rust 1.70 compatibility while the workspace targets Rust 1.80. Default features are disabled and only the `jpeg`, `png`, and `tiff` features are enabled. ICC embedding proof does not prove visual color correctness, and PNG/TIFF exports are currently explicit sRGB outputs without embedded ICC proof.
Binary size impact: Adds the selected JPEG/PNG/TIFF subset of `image` and its transitive codec support, including `png`, `tiff`, `jpeg-decoder`, and `weezl` through enabled codec features; measure final `.app` and `.dmg` during packaging QA.
...
Verification source: `cargo info image@0.25.6`, local Cargo metadata after Phase 5.4 and Task 20.2, Task 5.4 export tests, Task 5.6.4 thumbnail cache tests, Task 5.6.5 Loupe preview cache tests, Task 5.6.6 Develop preview tests, Task 11.7.3 metadata import tests, Task 13.6 ICC export tests, Task 20.2 PNG/TIFF export tests, and supported-raster source workflow tests.
```
