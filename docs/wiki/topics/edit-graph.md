...
- Task 29.1 audits the shipped Detail UI truth boundary: no listener or keyboard/delegated path can preview or commit Detail. The generic clipboard change listener includes the disabled Detail checkbox, but the selection builder requires `!disabled`, so Detail cannot enter clipboard mutation; persisted readback remains, and workflow smoke guards the callable boundary without changing UI pixels or desktop/backend Detail logic.
