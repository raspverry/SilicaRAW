...
- Sidecar rebuild dry-run scans `sidecars/` in deterministic order, resolves portable flags by `sidecar.flags`, then `edit_graph.metadata`, then defaults only after sidecar schema validation passes, and reports malformed sidecars, schema issues, photo-id mismatches, flag/metadata disagreements, and catalog reconciliation conflicts without applying changes.
...
## Phase 22 Library Scale Benchmarks

Task 22.2 adds local benchmark evidence for the paged Library query path. The report is generated by:

```bash
python3 scripts/harness/run-library-scale-benchmarks.py
```

The harness seeds synthetic catalog rows in a temporary local library, then measures the existing typed `silica-storage::query_library_photos` path for 1k, 10k, and 50k rows. It records machine metadata, dataset shape, query timings, and a lightweight render-adjacent page-model shaping timing.

The report is stored at [Library Scale Benchmarks](../reports/library-scale-benchmarks.md), with structured JSON at `docs/wiki/reports/library-scale-benchmarks.json`.

These timings are local observations only and are not universal performance guarantees.
