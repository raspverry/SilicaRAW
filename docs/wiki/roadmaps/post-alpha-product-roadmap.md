...
- **Status:** In progress on 2026-06-11. Task 11.5.1 added the typed paged query contract in `silica-catalog`; Task 11.5.2 raised the catalog schema to version 3 with normalized `photos.file_type` values and accepted query indexes; Task 11.5.3 added read-only storage/core paged query APIs; Task 11.5.4 exposed the typed desktop paged grid command. Page-scoped thumbnail hydration and UI consumption remain the next Task 11.5 slices.
