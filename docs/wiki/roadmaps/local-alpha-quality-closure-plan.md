...
- **Status:** Completed on 2026-07-08 as installed executable workflow evidence. Installed `/Applications/SilicaRAW.app` from the unsigned DMG, verified the installed app tree matches the mounted DMG app, launched it from `/Applications`, confirmed the process path is not the repository checkout, and ran the local alpha workflow through `/Applications/SilicaRAW.app/Contents/MacOS/silica-desktop`: create/open library, import by reference, grid review, blocked RAW placeholder, preview, exposure/contrast edit, JPEG sRGB export, restart persistence, and original-file safety. Evidence is recorded in [Local Alpha Installed App Launch](../reports/local-alpha-installed-app-launch.md) and [Local Alpha Installed App Workflow](../reports/local-alpha-installed-app-workflow.md). This does not prove WebView click paths, native path picker behavior, offline behavior, Gatekeeper, notarization, GitHub Release download, or clean-Mac behavior.
